        ...
        <GistContainer>
            <GistHeader>
                <Avatar>
                    ...
                    <UserNameLabel  href={url} target="_blank">{login}</UserNameLabel>
                </Avatar>
                <GistLinks>
            ...
            <GistDates>
                Created at {moment(created_at).format("DD/MM/YYYY")} {"  "} Last Updated {moment(updated_at).format("DD/MM/YYYY")}
            </GistDates>
            <GistDescription>
                {description}
            </GistDescription>
            <FilesList>
                {Object.keys(files).map(key => <FileItem href={files[key].raw_url} target="_blank"><Octicon name="file-text" code/>{" "}{key}</FileItem>)}
            </FilesList>
        </GistContainer>);
}
...
Gist.propTypes = {
    ...
    files: PropTypes.objectOf(PropTypes.oneOfType([
        PropTypes.string,
    ])),
    ...
    owner: PropTypes.objectOf(PropTypes.oneOfType([
        PropTypes.string,
        PropTypes.number,
        PropTypes.bool,
    ])),
